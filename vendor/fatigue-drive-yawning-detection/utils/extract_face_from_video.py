...
if __name__ == "__main__":
    ...
    video_dir = os.environ.get('YAWN_VIDEO_DIR', os.path.join(project_root, 'dataset', 'test_lst'))
    face_save_dir = os.environ.get('YAWN_FACE_SAVE_DIR', os.path.join(project_root, 'extracted_face', 'face_image'))
    if not os.path.exists(face_save_dir):
        os.makedirs(face_save_dir)
    train_file_list_dir = os.environ.get('YAWN_FACE_LIST_DIR', os.path.join(project_root, 'extracted_face'))
    if not os.path.exists(train_file_list_dir):
        os.makedirs(train_file_list_dir)
    file_list_name = os.environ.get('YAWN_FACE_LIST_NAME', 'test.txt')
    # dir_path, dir_names, _ = os.walk(video_dir)
    process(video_dir, face_save_dir, os.path.join(train_file_list_dir, file_list_name))
